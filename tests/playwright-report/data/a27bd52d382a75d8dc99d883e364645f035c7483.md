...
<body>
    ...
    <script>
        ...
        async function emptyRecycleBin() {
            ...
            if (!confirm(`⚠️ WARNING: This will PERMANENTLY delete ALL ${deletedData.length} transactions from the recycle bin view.\\n\\nNote: Data will be cleared from display but archived records remain secure in Supabase.`)) return;
            if (!confirm('Really sure? This will clear the recycle bin view!')) return;
            
            try {
                // Get all recycle bin IDs first
                const getResponse = await fetch(`${SUPABASE_URL}/rest/v1/recycle_bin?select=id`, {
                    method: 'GET',
                    headers: {
                        'apikey': SUPABASE_ANON_KEY,
                        'Authorization': `Bearer ${SUPABASE_ANON_KEY}`
                ...
                if (!getResponse.ok) {
                    ...
                    console.error('Empty Recycle Bin - Get IDs Error:', getResponse.status, errorText);
                    showMessage('❌ Error fetching recycle bin data: ' + errorText, 'error');
                    return;
                }
                
                const allIds = await getResponse.json();
                console.log('Found', allIds.length, 'items to delete');
                
                if (allIds.length === 0) {
                    showMessage('✅ Recycle bin is empty!', 'success');
                    loadDeletedData();
                    return;
                }
                
                // Delete each row individually (RLS-safe)
                let deleted = 0;
                let failed = 0;
                
                for (const row of allIds) {
                    try {
                        const deleteResponse = await fetch(`${SUPABASE_URL}/rest/v1/recycle_bin?id=eq.${row.id}`, {
                            method: 'DELETE',
                            headers: {
                                'apikey': SUPABASE_ANON_KEY,
                                'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
                        ...
                        if (deleteResponse.ok) {
                            ...
                            console.error('Failed to delete recycle item:', row.id, deleteResponse.status, errText);
                        }
                    } catch (e) {
                        failed++;
                        console.error('Exception deleting recycle item:', row.id, e);
                    }
                }
                
                console.log('Delete results - Success:', deleted, 'Failed:', failed);
                
                if (failed === 0) {
                    showMessage(`✅ Recycle bin cleared! ${deleted} items removed from view.`, 'success');
                } else if (deleted > 0) {
                    showMessage(`⚠️ Cleared ${deleted} items, ${failed} failed. Check console for details.`, 'error');
                } else {
                    showMessage(`❌ Failed to clear recycle bin. ${failed} items failed. Check console.`, 'error');
                }
...
- generic [ref=e2]:
  ...
  - table [ref=e18]:
    ...
    - rowgroup [ref=e28]:
      - row "1 10/5/2026, 1:11:12 pm 10/5/2026, 1:11:19 pm - Sample Data to delete ₹50.00 ♻️ Restore 🗑️ Delete" [ref=e29]:
        - cell "1" [ref=e30]
        - cell "10/5/2026, 1:11:12 pm" [ref=e31]
        - cell "10/5/2026, 1:11:19 pm" [ref=e32]
        - cell "-" [ref=e33]
        - cell "Sample Data to delete" [ref=e34]
        - cell "₹50.00" [ref=e35]
